...
SECRET_PATTERNS = [
    ...
    re.compile(
        r"""(?:AWS|AZURE|GCP|GITHUB|STRIPE|SLACK)_[A-Z_]*(?:KEY|SECRET|TOKEN)\s*[=:]\s*['"][^'"]+['"]""",
        re.IGNORECASE,
    ),
...
SQL_INJECTION_PATTERNS = [
    re.compile(
        r"""(?:execute|cursor\.execute|raw|rawQuery)\s*\(\s*f?['"].*\{""", re.IGNORECASE
    ),
    re.compile(
        r"""(?:execute|cursor\.execute)\s*\(\s*['"]\s*(?:SELECT|INSERT|UPDATE|DELETE).*%s""",
        re.IGNORECASE,
    ),
    re.compile(
        r"""\+\s*['"].*(?:SELECT|INSERT|UPDATE|DELETE|DROP|ALTER)""", re.IGNORECASE
    ),
]
...
def detect_large_functions(diff: str) -> list[dict]:
    ...
    for i, line in enumerate(lines):
        if not line.startswith("-"):
            func_match = re.match(
                r"^[+ ]?\s*(?:def|function|func|async def|public|private|protected)\s+(\w+)",
                line,
            )
            if func_match:
...
def calculate_complexity_score(files: list[dict], diff: str) -> dict:
    ...
    branch_keywords = len(
        re.findall(r"\b(?:if|else|elif|switch|case|for|while|catch|except|try)\b", diff)
    )
    logical_ops = len(re.findall(r"\b(?:and|or|&&|\|\|)\b", diff))

    cyclomatic_estimate = branch_keywords + logical_ops

    score = num_files * 2 + (total_changes // 50) + (cyclomatic_estimate // 10)
