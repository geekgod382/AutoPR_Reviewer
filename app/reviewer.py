...
async def handle_pr_event(payload: dict):
    try:
        ...
        # Run all analyses
        static_findings = await run_static_analysis(
            files, installation_id, owner, repo_name, commit_sha
        )
        ...
        summary = _format_summary(
            ai_result, static_findings, risk, files, premium_result, inline_comments
        )
        ...
        try:
            if inline_comments:
                ...
                await post_comment(
                    installation_id, owner, repo_name, pr_number, summary
                )
                logger.info(
                    "Posted summary comment for PR #%s (no inline comments)", pr_number
                )
        ...
        _record_review(
            installation_id, f"{owner}/{repo_name}", pr_number, risk["level"]
        )
...
def _format_summary(
    ...
    fallthrough_bugs = [
        i for i in ai_result.get("bugs", []) if not _was_inlined(i, inlined_keys)
    ]
    fallthrough_security = [
        i for i in ai_result.get("security", []) if not _was_inlined(i, inlined_keys)
    ]
    fallthrough_perf = [
        i for i in ai_result.get("performance", []) if not _was_inlined(i, inlined_keys)
    ]
    fallthrough_style = [
        i
        for i in ai_result.get("style_issues", [])
    ...
    if fallthrough_bugs:
        ...
        for bug in fallthrough_bugs:
            ...
            sections.append(
                f"- **[{sev}]** `{bug.get('file', '')}`: {bug['description']}"
            )
        sections.append("")
    ...
    sections.append("---")
    sections.append(
        "*Reviewed by [AutoPR Reviewer](https://github.com/apps/autopr-reviewer) 🤖*"
    )
...
def _record_review(
    ...
    try:
        ...
        if not installation:
            logger.warning(
                "Installation %s not found, skipping review log", github_installation_id
            )
            return
        log = ReviewLog(
